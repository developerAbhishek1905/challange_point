...
const [loading, setLoading] = useState(false);
  ...
  const getCategoriesList = async () => {
    try {
      const response = await dispatch(EventCategoryList({getAll:true}));
      console.log(response, 'evCategoryAllResponse');
      setEventDetails(response?.payload?.data?.data);
      setFirstCategory(response?.payload?.data?.data?.[0]?.category_id?.[0]?._id);
    } catch (error) {
      console.log(error);
    }
  };
...
useEffect(() => {
  if (isEdit && participant) {
    ...
    // setCategoryId(participant.category?._id || '');
...
// wait for eventDetails to load categories
    const selectedEvent = eventDetails.find(e => e._id === participant.event_id);
    if (selectedEvent && selectedEvent.category_id?.length > 0) {
      setCategoriesForEvent(selectedEvent.category_id);
      const foundCategory = selectedEvent.category_id.find(cat => cat._id === participant.category?._id);
      if (foundCategory) {
        setCategoryId(foundCategory._id);
        setIsResultDeclared(foundCategory?.is_result_declared ?? false);
      }
    }
...
//  if (participant.category?.is_result_declared) {
//   setIsResultDeclared(true);
// }

  }
 
}, [isEdit, participant,eventDetails]);
...
const validateForm = () => {
  ...
  if (!phoneNumber || phoneNumber.toString().length < 7) newErrors.phoneNumber = "Phone number is required";

  if (!selectedEventId) newErrors.selectedEventId = "Please select an event";
  if (!categoryId) newErrors.categoryId = "Please select a category";
  if (!bibNumber) newErrors.bibNumber = "Bib number is required";
  if (!wave.trim()) newErrors.wave = "Wave is required";
  if (!registrationId.trim()) newErrors.registrationId = "Registration ID is required";

  if (isResultDeclared) {
    if (!resultTIme) newErrors.resultTIme = "Result time is required";
    if (!rank) newErrors.rank = "Ranking is required";
    if (!resultStatus) newErrors.resultStatus = "Status is required";
...
const clearError = (field) => {
          ...
          <TabPane tab="Basic Information" key="1">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input className={inputClass} placeholder="First Name" value={firstName} onChange={(e)=>{setFirstName(e.target.value); clearError('firstName')}} />
                {errors.firstName && (
  <div className="text-red-500 text-xs mt-1">{errors.firstName}</div>
              ...
              <div>
                ...
                <Input className={inputClass} placeholder="Last Name" value={lastName} onChange={(e)=>{setLastName(e.target.value); clearError('lastName')}}/>
                {errors.lastName && (
  ...
  <div className="text-red-500 text-xs mt-1">{errors.gender}</div>
    ...
    onChange={(value) => {setNationality(value); clearError('nationality')}}
  ...
  <div className="text-red-500 text-xs mt-1">{errors.nationality}</div>
              ...
              <div>
                ...
                <InputNumber min={0} className={inputClass} placeholder="Id number" value={idNumber} onChange={(value)=>{setIDNumber(value); clearError('idNumber')}} />
                {errors.idNumber && (
  <div className="text-red-500 text-xs mt-1">{errors.idNumber}</div>
)}
              </div>
             <div>
  <label className={labelClass}>Email</label>
  <Input className={inputClass} value={email} placeholder="example@gmail.com" onChange={(e)=>{setEmail(e.target.value); clearError('email')}}/>
  {errors.email && (
  <div className="text-red-500 text-xs mt-1">{errors.email}</div>
              ...
              <div>
                ...
                <InputNumber className={inputClass} placeholder="999-999-9999" value={phoneNumber} onChange={(value)=>{setPhoneNumber(value); clearError('phoneNumber');}} onKeyPress={(e) => {
  ...
  <div className="text-red-500 text-xs mt-1">{errors.selectedEventId}</div>
)}
    </div>
    <div>
   ...
   clearError('categoryId');
    const selectedCategory = categoriesForEvent.find(cat => cat._id === value);
    setIsResultDeclared(selectedCategory?.is_result_declared ?? false);
  }}
        disabled={!categoriesForEvent.length}
        optionFilterProp="label"
      >
        {categoriesForEvent.map((category) => (
          <Select.Option key={category._id} value={category._id} label={`${category.category_name} `} >
  ...
  <div className="text-red-500 text-xs mt-1">{errors.categoryId}</div>
              ...
              <div>
                ...
                <InputNumber className={inputClass} placeholder="Bib Number" value={bibNumber} onChange={(value)=>{setBibNumber(value); clearError('bibNumber');}}  onKeyPress={(e) => {
  ...
  {errors.bibNumber && (
  <div className="text-red-500 text-xs mt-1">{errors.bibNumber}</div>
)}
              </div>
              <div>
                <label className={labelClass}>Wave</label>
                <Input className={inputClass} placeholder="Wave" value={wave} onChange={(e)=>{setWave(e.target.value); clearError('wave');}}/>
                {errors.wave && (
  <div className="text-red-500 text-xs mt-1">{errors.wave}</div>
)}
              </div>
              <div>
                <label className={labelClass}>Registration ID</label>
                <Input className={inputClass} placeholder="Registration ID" value={registrationId} onChange={(e)=>{setRegistrationId(e.target.value); clearError('registrationId')}} />
                {errors.registrationId && (
  <div className="text-red-500 text-xs mt-1">{errors.registrationId}</div>
)}
              </div>
            </div>
          </TabPane>
    ...
    <TabPane tab="Performance" key="3">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <TimePicker value={resultTIme} onChange={(time) => {setResultTime(time); clearError('resultTIme')}} use12Hours format="h:mm:ss.SSS A" className={inputClass} placeholder="result time" showTime={{ format: 'h:mm:ss.SSS A' }} />
            {errors.resultTIme && (
  <div className="text-red-500 text-xs mt-1">{errors.resultTIme}</div>
)}
        </div>
        <div>
          <label className={labelClass}>Ranking</label>
          <InputNumber min={1} className={inputClass} placeholder="Add ranking" value={rank} onChange={(value) => {setResultRank(value); clearError('rank')}}/>
          {errors.rank && (
  <div className="text-red-500 text-xs mt-1">{errors.rank}</div>
)}
        </div>
        <div>
    ...
    onChange={(value) => {setResultStatus(value); clearError('resultStatus');}}
    allowClear
    className="w-full"
    style={{
      height: '38px',
  ...
  {errors.resultStatus && (
  <div className="text-red-500 text-xs mt-1">{errors.resultStatus}</div>
)}
        </div>
        <div>
          <label className={labelClass}>Remarks</label>
          {/* <input className={inputClass} placeholder="Add remarks" /> */}
          <Input placeholder="add remarks" className="h-9" value={remark} onChange={(e) => {setResultRemark(e.target.value); clearError('remark')}}/>
          {errors.remark && (
  <div className="text-red-500 text-xs mt-1">{errors.remark}</div>
    ...
    loading ? "bg-blue-300 cursor-not-allowed" : "bg-blue-600 text-white"
      ...
      {/* {loading && (
  <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-60 rounded-2xl z-50">
    <span className="loader border-4 border-t-blue-600 rounded-full w-8 h-8 animate-spin"></span>
  </div>
)} */}
    </div>
  );
