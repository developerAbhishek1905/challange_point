        ...
        <div className="overflow-x-auto rounded-xl ">
          <table className="min-w-full border-collapse ">
            <thead className="bg-gray-100">
              <tr>
                {["Title", "Uploader", "Location", "Views", "Reported",  "Actions"].map(
                  (header) => (
            ...
            <tbody>
              {pagedFeeds.length > 0 ? (
                pagedFeeds.map((feed) => {
                  ...
                  return (
                    <motion.tr key={feed._id} className="hover:bg-gray-50 border-b text-gray-800 ">
                      <td className="px-6 py-3 text-sm font-medium">{feed.title}</td>
                      <td className="px-6 py-3 text-sm">{feed.uploadedBy?.name || "N/A"}</td>

                      {/* ✅ Location with popover */}
                      <td className="px-6 py-3 text-sm">
                        <Popover content={<span className="max-w-xs block">{address}</span>} title="Full Location" trigger="click">
                ...
                <tr>
                  <td colSpan={6} className="text-center py-8">
                    <Empty description="No feeds found" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {viewModalOpen && feedToView && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-5xl h-[90vh] overflow-y-auto relative">
            <button onClick={() => setViewModalOpen(false)} className="absolute top-4 right-4  text-gray-600 rounded-full px-3 py-1 text-lg">✕</button>
      ...
      {feedToDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl w-full max-w-md">
            ...
            <p className="text-sm text-gray-600 mb-4">Are you sure you want to delete this feed? This action cannot be undone.</p>

            <div className="flex justify-end gap-3">
              <button className="px-4 py-2 rounded-md border text-gray-700" onClick={() => setFeedToDelete(null)}>Cancel</button>

              <button className="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-700" onClick={handleDelete}>Delete</button>
            </div>
          </div>
        </div>
      )}
